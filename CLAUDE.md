...
## Autonomous / Long Session Protocol

Applies to every unattended phase run, and to any interactive session past roughly an hour. Compaction dilutes instructions: a session that obeys these rules at minute 10 starts ignoring them at minute 90 unless the rules stay cheap to hold.

### Context budget

1. **Redirect gate output to a file, then tail it. Never run a gate bare.**
   ```
   npm run lint:a11y > "$RUN/a11y.log" 2>&1; tail -20 "$RUN/a11y.log"
   ```
   The render gates (`measure_render.mjs`, `verify_states.mjs`, `axe_audit.mjs`, `accuracy_report.mjs`) print per-element output for every state in both themes. One bare run of `accuracy_report` can eat a large share of the window; the full log stays on disk for when a failure needs reading.
2. **Never re-read a file already in context.** Grep for the lookup.
3. **Report numbers, not transcripts.** A gate's `N/N` line and its failing rows, not its output.
4. **Screenshots are expensive.** One per state you are actually judging, not one per step. The RENDER AND LOOK rule still applies, it just does not license a screenshot per click.

### Handoff

- `STATUS.md` at repo root is the handoff document. Read it at session start. Update it before ending a session or a phase.
- Record: phase completed, findings as `file:line`, what is next. Never paste raw logs into it.
- Phases communicate through `STATUS.md` and files on disk, never through conversation history. Treat a run like a CI pipeline, not a conversation.

### Stop conditions (unattended runs)

Stop work, update `STATUS.md`, and exit if **any** of these hold:

- **Claude usage has reached 60 percent of the weekly limit.** Check before starting each phase, and again after any output that mentions limits or quota. Sixty percent is the ceiling with headroom deliberately left for the next day's interactive work; do not spend into the last 40 percent unattended.
- `qa/STOP` exists (the kill switch: `touch qa/STOP` from any shell).
- A phase would need to write production data to continue.

A phase that fails twice does **not** stop the run. Phases are independent: the
orchestrator records it as `phase<N>.FAILED` and moves to the next one, because a
dead Android build at 2am should not cost you the retrospective at 5am.

Never work past a stop condition, and never resume one on your own judgement. A half-finished run with an honest `STATUS.md` is worth more than a locked-out account at 6am.

### Data safety (unattended runs)

- Production Firestore is **read-only**. No QA phase writes data, ever.
- `read_firestore.js` is the read-only shape. Do not add a write twin, and do not run any script that calls `setDoc`, `updateDoc`, `deleteDoc`, or `writeBatch` against production.
- Cleanup is **written, never executed**: put it in `qa/<run>/cleanup.md` for a human to read and run.
- Inspection phases do not edit source. Only the designated fix phase writes code, and it commits per fix.
