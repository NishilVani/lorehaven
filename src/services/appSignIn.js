// A Steam sign-in that starts in the desktop or Android app.
//
// The app cannot be Steam's return address: Steam only comes back to
// https://lorehaven.app, and that page hands the result to the app through a
// lorehaven:// link the operating system routes. Another program can register
// for that link too, so the app keeps a random verifier and sends Steam only its
// SHA-256, inside the return address Steam signs. The Worker then accepts the
// sign-in only from whoever holds the verifier -- this app.
//
...
const APP_ROUTES = ['/auth/steam', '/import/steam'];

const VERIFIER_KEY = 'lorehaven_steam_verifier';
...
export function keepVerifier(verifier, now = Date.now()) {
  try { localStorage.setItem(VERIFIER_KEY, JSON.stringify({ verifier, at: now })); } catch { /* storage refused */ }
}

/** The kept verifier, removed as it is read: a verifier is good for one sign-in. */
export function takeVerifier(now = Date.now()) {
  try {
