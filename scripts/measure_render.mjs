...
catch { console.log('measure_render: playwright not installed — SKIPPED (npm i -D playwright to enable)'); process.exit(2); }
...
if (files.length === 0) { console.log('measure_render: no HTML files to check — SKIPPED'); process.exit(2); }
...
catch { try { browser = await chromium.launch(); } catch (e) { console.log('measure_render: no browser available — SKIPPED'); process.exit(2); } }
