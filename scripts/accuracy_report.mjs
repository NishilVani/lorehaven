...
const checks = [
  ['Token JSON valid + aliases resolve', 'python3 scripts/validate_tokens.py || python scripts/validate_tokens.py'],
  ['WCAG contrast — token pairs (light + dark)', 'python3 scripts/validate_contrast.py || python scripts/validate_contrast.py'],
  ['Component specs complete (anatomy/variants/states/tokens/a11y)', 'python3 scripts/validate_component_spec.py || python scripts/validate_component_spec.py'],
  ['No hardcoded values (hex/px/ms/Tailwind/font) — golden', 'python3 scripts/lint_hardcodes.py examples/golden || python scripts/lint_hardcodes.py examples/golden'],
  ['No hardcoded values — sample-app', 'python3 scripts/lint_hardcodes.py examples/sample-app || python scripts/lint_hardcodes.py examples/sample-app'],
  ['Every var(--…) resolves to the theme (no floating tokens)', 'python3 scripts/validate_theme_refs.py || python scripts/validate_theme_refs.py'],
  ['No emoji in UI output, taste docs, or the agent instruction surface', 'python3 scripts/check_no_emoji.py || python scripts/check_no_emoji.py'],
