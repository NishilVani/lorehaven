...
Usage:
  ...
  python3 scripts/check_no_emoji.py --no-dash src        # emoji only, keep em-dashes
Exit 0 = clean, 1 = an emoji/pictograph was found.

--no-dash exists so this can gate a real product tree. The em-dash rule is a
house-style heuristic, not the emoji ban: LoreHaven's voice is deliberately
editorial and uses em-dashes throughout its copy and comments (337 of them), so
without the flag the emoji signal drowns and nobody wires the check up at all.
"""
import re
import sys
...
def main(argv):
    check_dash = "--no-dash" not in argv
    argv = [a for a in argv if a != "--no-dash"]
    ...
    for f in files:
        try:
            ...
            continue
        ui = check_dash and f.suffix in UI_EXTS
        for n, line in enumerate(text.splitlines(), 1):
            for m in EMOJI.finditer(line):
                # ascii(), not !r: on a cp1252 console printing the character itself
                # raised UnicodeEncodeError, so the gate CRASHED on exactly the input
                # it exists to catch. Escapes always print.
                hits.append(f"{f}:{n}: emoji/pictograph {ascii(m.group(0))} - use lucide / plain text")
            if ui:
