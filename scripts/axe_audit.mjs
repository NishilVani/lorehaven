...
catch { console.log('axe_audit: playwright not installed — SKIPPED'); process.exit(2); }

const argv = process.argv.slice(2);
const dark = argv.includes('--dark');
const file = argv.find(a => !a.startsWith('--'));
if (!file) { console.log('usage: node scripts/axe_audit.mjs <file.html> [--dark]'); process.exit(2); }

const browser = await chromium.launch({ channel: 'chrome' }).catch(() => chromium.launch());
const page = await browser.newPage({ viewport: { width: 1280, height: 900 } });
await page.goto(pathToFileURL(resolve(file)).href, { waitUntil: 'networkidle' }).catch(async (e) => {
  console.log(`axe_audit: could not open ${file} — ${e.message} — SKIPPED`);
